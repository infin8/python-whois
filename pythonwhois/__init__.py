...
def get_whois(domain, normalized=[], interface=None, proxy=None):
	raw_data, server_list = net.get_whois_raw(domain, with_server_list=True, interface=interface, proxy=proxy)
	...
	return parse.parse_raw_whois(raw_data, normalized=normalized, never_query_handles=False, handle_server=server_list[-1], interface=interface, proxy=proxy)
